...
function wrapWithPromise( staticFn, args ) {
  var promiseFn = _.wrap( staticFn, function( fn ) {
    var token = Q.defer();
    args = args.concat([token.resolve, token.reject]);
    fn.apply( null, args );
...
function wrapAnySyncFunctionsWithBooleanPromiseHandling( fns, options ) {
  options = ( options || {} );
...
function wrapAnySequenceFunctionsThatRequestedPromises( fns, options ) {
  options = ( options || {} );
  ...
  return _.chain( fns )
    ...
    .map(function(fn, index) {
      return function( res ) {
        ...
        if ( index >= 1 && !options.preventResultChaining ) args.push(res);
        ...
        if (( index === 0 && fn.length >= 1 ) || ( index >= 1 && fn.length >= 2 )) {
          fn = wrapWithPromise( fn, args );
        }
